...
function move() {
    ...
    if (!isSnakeUnit(new_unit)  /* && new_unit !== undefined* Змейка не ушла за границу поля*/ ) {

        if (new_unit == undefined){

            //alert("undefined"); Змейка ушла за границу поля
...
function createWall() {
    ...
    while (!wallCreated) { //пока стенку не создали
        ...
        wall_cell = document.getElementsByClassName('cell-' + wall_y + '-' + wall_x)[0];
        var wall_cell_classes = wall_cell.getAttribute('class').split(' ');

        // проверка на змейку или еду
    ...
    if (wallCreated==true){
        setTimeout(removeWall, 10000); 
    }

    function removeWall(){
        wall_cell.setAttribute('class', classes + ' ');
    }
